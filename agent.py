...
def run_agent(query: str, wardrobe: dict) -> dict:
    ...
    # Guard against empty/missing description
    if not parsed.get("description"):
        session["error"] = (
            "What are you looking for? Try something like "
            "'vintage jeans under $40, size M'."
        )
        return session
    ...
    # Branch: if no results, stop here — do NOT call suggest_outfit
    if not results:
        session["error"] = (
            "No listings found matching your criteria. Try broadening your search "
            "(different keywords, larger size range, or higher budget)."
        )
        return session
